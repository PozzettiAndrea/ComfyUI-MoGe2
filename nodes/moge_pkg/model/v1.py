...
class ResidualConvBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int = None, hidden_channels: int = None, padding_mode: str = 'replicate', activation: Literal['relu', 'leaky_relu', 'silu', 'elu'] = 'relu', norm: Literal['group_norm', 'layer_norm'] = 'group_norm', dtype=None, device=None, operations=ops):
...
class Head(nn.Module):
    def __init__(
        ...
        self.projects = nn.ModuleList([
            operations.Conv2d(in_channels=dim_in, out_channels=dim_proj, kernel_size=1, stride=1, padding=0, dtype=dtype, device=device) for _ in range(num_features)
        ])

        self.upsample_blocks = nn.ModuleList([
            nn.Sequential(
                self._make_upsampler(in_ch + 2, out_ch),
                *(ResidualConvBlock(out_ch, out_ch, dim_times_res_block_hidden * out_ch, activation="relu", norm=res_block_norm, dtype=dtype, device=device, operations=operations) for _ in range(num_res_blocks))
    ...
    def _make_upsampler(self, in_channels: int, out_channels: int):
        ...
        upsampler = nn.Sequential(
            operations.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2, dtype=self._dtype, device=self._device),
            operations.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, padding_mode='replicate', dtype=self._dtype, device=self._device)
        )
    ...
    def _make_output_block(self, dim_in: int, dim_out: int, dim_times_res_block_hidden: int, last_res_blocks: int, last_conv_channels: int, last_conv_size: int, res_block_norm: Literal['group_norm', 'layer_norm']):
        ...
        return nn.Sequential(
            operations.Conv2d(dim_in, last_conv_channels, kernel_size=3, stride=1, padding=1, padding_mode='replicate', dtype=self._dtype, device=self._device),
            *(ResidualConvBlock(last_conv_channels, last_conv_channels, dim_times_res_block_hidden * last_conv_channels, activation='relu', norm=res_block_norm, dtype=self._dtype, device=self._device, operations=operations) for _ in range(last_res_blocks)),
            nn.ReLU(inplace=True),
            operations.Conv2d(last_conv_channels, dim_out, kernel_size=last_conv_size, stride=1, padding=last_conv_size // 2, padding_mode='replicate', dtype=self._dtype, device=self._device),
        )
...
class MoGeModel(nn.Module):
    ...
    def __init__(self, 
        ...
        **deprecated_kwargs
        ...
        self.backbone = hub_loader(pretrained=False, dtype=dtype, device=device, operations=operations)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        if Path(pretrained_model_name_or_path).exists():
            checkpoint = comfy.utils.load_torch_file(str(pretrained_model_name_or_path), safe_load=True)
        else:
            ...
            checkpoint = comfy.utils.load_torch_file(str(cached_checkpoint_path), safe_load=True)
    ...
    def forward(self, image: torch.Tensor, num_tokens: int) -> Dict[str, torch.Tensor]:
        ...
        # Output post-processing (originally wrapped in torch.autocast(fp32);
        # comfy.ops handles dtype management at the layer level, and our
        # forward already runs in operations-managed precision -- explicit
        # autocast becomes redundant).
        points = points.float()
        mask = mask.float()
    ...
    def infer(
        ...
        # Forward pass. The original code wrapped this in
        #   torch.autocast(dtype=torch.float16, enabled=use_fp16 and self.dtype != fp16)
        # which the ComfyUI-native lint forbids. comfy.ops layers handle dtype
        # casting per-call via cast_bias_weight; users can request fp16 explicitly
        # via the loader's `precision` widget (it sets self.dtype to float16).
        if use_fp16 and self.dtype != torch.float16:
            image = image.to(dtype=torch.float16)
        output = self.forward(image, num_tokens)
        points, mask = output['points'], output['mask']

        # Always process the output in fp32 precision (autocast removed; explicit .float()).
